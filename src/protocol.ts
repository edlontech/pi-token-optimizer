...
export function isLimit(value: unknown): value is number {
  return (
    ...
    (value as number) >= 1 &&
    (value as number) <= MAX_EXPANSION_LINES
  );
}
...
function isNonemptyString(
  value: unknown,
  bytes = MAX_DESCRIPTOR_STRING_BYTES,
): value is string {
  return (
    typeof value === "string" && value.trim().length > 0 && fits(value, bytes)
  );
}
...
function isJsonValue(
  ...
  if (Array.isArray(value)) {
    ...
    valid =
      ...
      entries.every(
        ([key, item]) =>
          fits(key, 256) && isJsonValue(item, depth + 1, ancestors),
      );
  } else {
    valid = false;
  }
...
function isSessionDescriptor(value: unknown): value is SessionDescriptor {
  if (!isRecord(value) || !hasOnlyKeys(value, SESSION_KEYS) || !isId(value.id))
    return false;
...
function hasRequiredRequestFields(request: BridgeRequest): boolean {
  switch (request.action) {
    ...
    case "expand": {
      ...
      if (args.offset !== undefined && !isOffset(args.offset)) return false;
...
export function isBridgeRequest(value: unknown): value is BridgeRequest {
  if (
    ...
    (value.args !== undefined &&
      (!isRecord(value.args) || !isJsonValue(value.args)))
  ) {
    return false;
  }
  ...
  if (
    request.tool !== undefined &&
    request.action !== "pre_tool" &&
    request.action !== "post_tool"
  )
    return false;
  return (
    hasRequiredRequestFields(request) &&
    encodedSizeAtMost(request, MAX_REQUEST_BYTES)
  );
}
...
function isContext(value: unknown, action: BridgeAction): boolean {
  ...
  return (
    (action === "before_prompt" && value.scope === "nudge") ||
    (action === "session_start" && value.scope === "recovery")
  );
}

function responseFieldsFitAction(
  response: BridgeResponse,
  action: BridgeAction,
): boolean {
  if (response.updatedInput !== undefined && action !== "pre_tool")
    return false;
  if (
  ...
  if (action === "expand" && response.ok) {
    ...
    if (
      !isBoundedString(text, MAX_EXPANSION_TEXT_BYTES) ||
      text.split("\n").length > MAX_EXPANSION_LINES ||
      (nextOffset !== undefined &&
        (!Number.isSafeInteger(nextOffset) || (nextOffset as number) < 0))
    )
      return false;
  }
  return true;
...
export function isBridgeResponse(
  ...
  if (
    ...
    (value.updatedInput !== undefined &&
      (!isRecord(value.updatedInput) || !isJsonValue(value.updatedInput))) ||
    (value.replacementText !== undefined &&
      !isNonemptyString(value.replacementText, MAX_RESPONSE_BYTES)) ||
    (value.archiveId !== undefined && !isId(value.archiveId)) ||
    (value.data !== undefined &&
      (!isRecord(value.data) || !isJsonValue(value.data))) ||
    (value.errorCode !== undefined &&
      (!isNonemptyString(value.errorCode, MAX_ID_LENGTH) ||
  ...
  if (
    (value.ok && value.errorCode !== undefined) ||
    (!value.ok && value.errorCode === undefined)
  )
    return false;
  if (
  ...
  return (
    responseFieldsFitAction(value as unknown as BridgeResponse, action) &&
    encodedSizeAtMost(value, MAX_RESPONSE_BYTES)
  );
}

export function isHealthyStatus(response: unknown): boolean {
  return (
    isBridgeResponse(response, "status") &&
    response.ok &&
...
export function validateBridgeResponse(
  ...
  if (!isBridgeResponse(value, action))
    throw new TypeError("Invalid bridge response");
  return value;
}
