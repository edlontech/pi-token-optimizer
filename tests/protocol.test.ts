...
test("rejects unknown versions, actions, and descriptor fields", () => {
  assert.equal(
    isBridgeRequest({ ...validRequestByAction.status, protocolVersion: 2 }),
    false,
  );
  assert.equal(
    isBridgeRequest({ ...validRequestByAction.status, action: "install" }),
    false,
  );
  assert.equal(
    isBridgeRequest({ ...validRequestByAction.status, unexpected: true }),
    false,
  );
  assert.equal(
    isBridgeRequest({
...
test("rejects invalid ids and oversized descriptor strings", () => {
  ...
  assert.equal(
    isBridgeRequest({
      ...
      session: {
        ...session,
        file: "x".repeat(MAX_DESCRIPTOR_STRING_BYTES + 1),
      },
    }),
    false,
  );
...
test("requires a bounded session working directory", () => {
  ...
  assert.equal(
    isBridgeRequest({ ...validRequestByAction.status, session: withoutCwd }),
    false,
  );
  assert.equal(
    isBridgeRequest({
...
test("rejects malformed and oversized JSON payload fields", () => {
  ...
  assert.equal(
    isBridgeRequest({ ...validRequestByAction.status, args: new Date() }),
    false,
  );
  assert.equal(
    isBridgeRequest({
...
test("rejects JSON at depth 20", () => {
  ...
  assert.equal(
    isBridgeRequest({ ...validRequestByAction.status, args: nested }),
    false,
  );
});
...
test("rejects requests missing action requirements", () => {
  assert.equal(
    isBridgeRequest({ protocolVersion: 1, action: "pre_tool", session }),
    false,
  );
  assert.equal(
    isBridgeRequest({ protocolVersion: 1, action: "post_tool", session, tool }),
    false,
  );
  assert.equal(
    isBridgeRequest({
...
test("requires complete post-tool result metadata", () => {
  ...
  assert.equal(
    isBridgeRequest({ ...base, args: { text: "ok", hasImages: false } }),
    false,
  );
  assert.equal(
    isBridgeRequest({ ...base, args: { text: "ok", isError: false } }),
    false,
  );
  assert.equal(
    isBridgeRequest({
  ...
  const responses = [
    ...
    [
      "before_prompt",
      {
        protocolVersion: 1,
        ok: true,
        contexts: [{ scope: "nudge", text: "focus" }],
      },
    ],
    [
      "session_start",
      {
        protocolVersion: 1,
        ok: true,
        contexts: [{ scope: "recovery", text: "resume" }],
      },
    ],
    [
      "expand",
      { protocolVersion: 1, ok: true, data: { text: "slice", nextOffset: 5 } },
    ],
    [
      "doctor",
      { protocolVersion: 1, ok: false, errorCode: "python_unavailable" },
    ],
  ] as const;
...
test("requires post-tool replacement text and archive id together", () => {
  assert.equal(
    isBridgeResponse({ protocolVersion: 1, ok: true }, "post_tool"),
    true,
  );
  assert.equal(
    isBridgeResponse(
...
test("rejects malformed and action-inappropriate responses", () => {
  assert.equal(
    isBridgeResponse({ protocolVersion: 2, ok: true }, "status"),
    false,
  );
  assert.equal(
    isBridgeResponse(
      { protocolVersion: 1, ok: true, surprise: true },
      "status",
    ),
    false,
  );
  assert.equal(
    isBridgeResponse({ protocolVersion: 1, ok: false }, "status"),
    false,
  );
  assert.equal(
    isBridgeResponse(
      { protocolVersion: 1, ok: true, errorCode: "bad" },
      "status",
    ),
    false,
  );
  assert.equal(
    isBridgeResponse(
      { protocolVersion: 1, ok: true, updatedInput: {} },
      "status",
    ),
    false,
  );
